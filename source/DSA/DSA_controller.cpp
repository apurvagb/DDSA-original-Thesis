...
//    argos::TConfigurationNode settings = argos::GetNode(node, "");
...
//    argos::GetNodeAttribute(settings, "RandomSeed",          Random_Seed);
//    argos::GetNodeAttribute(settings, "ResultsDirectoryPath",          result_path);
...
//    ss << "DSA-"<<NumberOfRobots<<"_"<<Random_Seed<<"-"
//       << (now->tm_year) << '-'
//       << (now->tm_mon + 1) << '-'
//       <<  now->tm_mday << '-'
//       <<  now->tm_hour << '-'
...
//    results_full_path = result_path+"/"+results_file_name;
...
void DSA_controller::ControlStep() 
{
  ...
  // To draw paths only for going in/out of nest
